import { Link } from "react-router-dom";
import { useGetCommentsQuery } from "../services/Server";
import { CommentBox } from "./CommentBox";
import { CommentCard } from "./CommentCard";
import { Loading } from "./loading";
import { useState } from "react";
import { handleLogout } from "../utilities/handleLogout";

export const Home = () => {
  const [isLoggedin, setLoggedin] = useState(
    localStorage.getItem("isLoggedin")
  );
  const { data, error, isLoading } = useGetCommentsQuery();
  if (isLoading) {
    return (
      <div className="w-dvw h-dvh flex justify-center items-center">
        <Loading />
      </div>
    );
  }
  if (error) {
    return <div>error</div>;
  }
  if (data) {
    return (
      <div className="w-full min-h-dvh h-full bg-neutral-200 flex flex-col items-center gap-5 p-5">
        <div className="w-full flex justify-end items-center">
          {isLoggedin ? (
            <button
              onClick={handleLogout}
              className="rounded-md bg-neutral-800 py-1.5 px-4.5 text-lg font-semibold text-white cursor-pointer"
            >
              Logout
            </button>
          ) : (
            <button className="rounded-md bg-neutral-800 py-1.5 px-4.5 text-lg font-semibold text-white cursor-pointer">
              <Link to={"/login"}>Login</Link>
            </button>
          )}
        </div>
        {data.map((comment, index) => {
          return (
            <CommentCard
              username={comment.username}
              body={comment.body}
              id={comment.id}
              key={index}
            />
          );
        })}
        <CommentBox data={data} />
      </div>
    );
  }
};
